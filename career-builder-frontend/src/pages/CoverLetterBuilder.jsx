...
// Memoize components to prevent re-render
const QuestionBox = memo(({ title, children }) => (
...
const OptionButton = memo(({ label, active, onClick }) => (
  <button
    onClick={onClick}
...
export default function CoverLetterBuilder() {
  ...
  const schoolOptions = ["High School", "Trade School", "College", "Graduate School"];
  ...
  const generateLetter = () => {
    ...
    const intro = `I am excited to apply for the ${role} position at ${company}. With ${experience.toLowerCase()} of professional experience and a background in ${study || "a related field"}, I am eager to contribute to ${company}’s mission of ${companyFocus || "driving innovation and growth"}.\n`;

    const experienceText = experience === "No Experience"
      ...
      : `Throughout my career, I have built expertise in ${skills || "leadership, communication, and problem-solving"}. These experiences have enabled me to refine my professional and technical abilities while consistently achieving positive results.`;

    const studentText = student === "Yes" || student === "Recent Graduate"
      ...
      <div className="max-w-4xl w-full">
        ...
        {step === 1 && (
          <QuestionBox title="How long have you been working?">
            <p className="text-gray-500 mb-6">We’ll find the best templates for your experience level.</p>
            <div className="flex flex-wrap justify-center gap-6">
              {experienceOptions.map(option => (
                <OptionButton key={option} label={option} active={experience === option} onClick={() => { setExperience(option); setStep(2); }} />
              ))}
            </div>
          </QuestionBox>
        )}
          ...
          <QuestionBox title="Are you a student?">
            <div className="flex flex-wrap justify-center gap-6">
              {studentOptions.map(option => (
                <OptionButton key={option} label={option} active={student === option} onClick={() => { setStudent(option); setStep(3); }} />
              ))}
            </div>
          </QuestionBox>
        )}
          ...
          <QuestionBox title="What kind of school is it?">
            <div className="flex flex-wrap justify-center gap-6">
              {schoolOptions.map(option => (
                <OptionButton key={option} label={option} active={schoolType === option} onClick={() => { setSchoolType(option); setStep(4); }} />
              ))}
            </div>
          </QuestionBox>
        )}
        ...
        {step === 3 && student === "No" && (
          <QuestionBox title="Did you graduate from high school?">
            <div className="flex flex-wrap justify-center gap-6">
              {["Yes", "No"].map(option => (
                <OptionButton key={option} label={option} active={graduated === option} onClick={() => { setGraduated(option); setStep(4); }} />
              ))}
            </div>
          </QuestionBox>
        )}

        {/* Step 4 — Study + Role + Company */}
        {step === 4 && (
          <QuestionBox title="What did you study?">
            ...
            />
            <h2 className="text-3xl font-semibold mt-12">What company and role are you applying for?</h2>
        ...
        {/* Step 6 — Company Focus */}
        {step === 6 && (
          <QuestionBox title="What do you admire or know about the company?">
            <textarea value={companyFocus} onChange={e => setCompanyFocus(e.target.value)}
        ...
        {/* Step 9 — Generated Letter */}
        {step === 9 && generatedLetter && (
          <div className="mt-16 bg-gray-50 border border-gray-200 rounded-lg p-8 shadow-md">
            <h2 className="text-2xl font-bold text-blue-600 mb-6 text-center">
              Your Generated Cover Letter
            </h2>
            <pre className="whitespace-pre-wrap text-left text-gray-800 text-lg leading-relaxed">{generatedLetter}</pre>
